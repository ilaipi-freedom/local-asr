...
step "3/6 部署运行脚本"
# 优先用仓库内的 service/（本仓库场景）；否则用 skill 自带的 assets（换机器场景）
SRC="$SKILL_DIR/assets/pi-asr"
[ -d "$SKILL_DIR/../service" ] && [ -f "$SKILL_DIR/../service/qwen_asr_server.py" ] && SRC="$SKILL_DIR/../service"
echo "   来源: $SRC"
cp -f "$SRC"/*.py "$ASR_DIR"/ || die "复制脚本失败"
[ -f "$SRC/README.md" ] && cp -f "$SRC/README.md" "$ASR_DIR/"
chmod +x "$ASR_DIR"/transcribe_qwen.py "$ASR_DIR"/qwen_asr_server.py
if [ "$DO_W" = 1 ]; then
  mkdir -p "$W_DIR"
  for cand in "$SKILL_DIR/../service/whisper/transcribe.py" "$SKILL_DIR/assets/pi-whisper/transcribe.py"; do
    [ -f "$cand" ] && cp -f "$cand" "$W_DIR/" && break
  done
  if [ ! -x "$W_DIR/venv/bin/python" ]; then
    python3 -m venv "$W_DIR/venv" && "$W_DIR/venv/bin/pip" -q install faster-whisper || echo "⚠️ whisper 兜底安装失败（可选，可忽略）"
  fi
fi
...
PY
  ...
  echo "跳过（无 telegram.json 或 --no-telegram）"
fi

echo
echo "完成。自检: $SKILL_DIR/scripts/doctor.sh"
